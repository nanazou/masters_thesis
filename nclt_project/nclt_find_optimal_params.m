% nclt_find_optimal_params.m
% このスクリプトは、NCLT政策の最適パラメータを探索し、
% 厚生の表面グラフを出力します。計算の核心部は src/nclt_solve_core.m を呼び出します。
...
fprintf('====================================================\n');
fprintf('Starting optimization for NCLT Policy...\n');
fprintf('====================================================\n\n');
...
% --- 4. 粗い探索 (coarse mode) の設定 --- beta final
%phi_gap_H_range_coarse   = 120:10:140;
%phi_level_H_range_coarse = 130:10:150;

% --- 4. 粗い探索 (coarse mode) の設定 --- a final
%phi_gap_H_range_coarse   = 97:1:99;
%phi_level_H_range_coarse = 105:5:115;

% --- 5. 詳細な探索 (fine mode) の設定 --- beta final
%center_phi_gap_H   = 130; 
%center_phi_level_H = 140; 
%fine_search_radius_gap_H   = 0.2; 
%fine_search_step_gap_H     = 0.1; 
%fine_search_radius_level_H = 0.2; 
%fine_search_step_level_H   = 0.1;

% --- 5. 詳細な探索 (fine mode) の設定 --- a final
%center_phi_gap_H   = 98; 
%center_phi_level_H = 110.2; 
%fine_search_radius_gap_H   = 0.2; 
%fine_search_step_gap_H     = 0.1; 
%fine_search_radius_level_H = 0.2; 
%fine_search_step_level_H   = 0.1; 
...
% 独自名に変更した統一ファイル名 ('nclt_model', 'nclt_model_zlb') を参照
settings.modnam = 'nclt_model';
settings.modnamstar = 'nclt_model_zlb';
...
for i = 1:length(phi_level_H_range_final)
    for j = 1:length(phi_gap_H_range_final)
        ...
        fprintf('\nTrial (%d/%d): phi_gap_H = %.4f, phi_level_H = %.4f ... ', ...
            current_iteration, total_iterations, phi_gap_H_value, phi_level_H_value);

        % --- [計算核心 nclt_solve_core を呼び出す] ---
        try
            % 接頭辞を統一したコアエンジンを呼び出し
            res = nclt_solve_core(phi_gap_H_value, phi_level_H_value, settings);

            % 命名規則 (with_delta_pie) に基づいて結果を取得
...
% =========================================================================
fprintf('\n--- NCLT Final Optimal Policy Results (Shock: %s) ---\n', shock_scenario);
fprintf('1. Optimal for Home:\n');
fprintf('   Parameters: phi_gap_H = %.4f, phi_level_H = %.4f\n', final_results.opt_phi_gap_H_H, final_results.opt_phi_level_H_H);
...
h_home = figure('Name', 'NCLT: Home Welfare', 'Visible', 'off');
surf(phi_gap_H_range_final, phi_level_H_range_final, results_welfare_H_final);
colorbar; view(3); hold on;
plot3(final_results.opt_phi_gap_H_H, final_results.opt_phi_level_H_H, final_results.max_welfare_H, ...
...
line2_H = sprintf('\\phi_{gap}^H=%.4f, \\phi_{level}^H=%.4f, Welfare=%.4f', ...
...
h_foreign = figure('Name', 'NCLT: Foreign Welfare', 'Visible', 'off');
surf(phi_gap_H_range_final, phi_level_H_range_final, results_welfare_F_final);
colorbar; view(3); hold on;
plot3(final_results.opt_phi_gap_H_F, final_results.opt_phi_level_H_F, final_results.max_welfare_F, ...
      'r*', 'MarkerSize', 12, 'LineWidth', 1.5);
z_limits_F = zlim;
...
line2_F = sprintf('\\phi_{gap}^H=%.4f, \\phi_{level}^H=%.4f, Welfare=%.4f', ...
...
h_global = figure('Name', 'NCLT: Global Welfare', 'Visible', 'off');
surf(phi_gap_H_range_final, phi_level_H_range_final, results_welfare_SUM_final);
colorbar; view(3); hold on;
plot3(final_results.opt_phi_gap_H_SUM, final_results.opt_phi_level_H_SUM, final_results.max_welfare_SUM, ...
      'r*', 'MarkerSize', 12, 'LineWidth', 1.5);
z_limits_G = zlim;
...
line2_G = sprintf('\\phi_{gap}^H=%.4f, \\phi_{level}^H=%.4f, Welfare=%.4f', ...
...
fprintf('\n====================================================\n');
fprintf('NCLT Policy optimization has finished for %s.\n', shock_scenario);
