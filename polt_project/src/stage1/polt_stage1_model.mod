// src/stage1/polt_stage1_model.mod

// Stage 1 専用の宣言ファイルを読み込む
@#include "polt_stage1_model_declarations.inc"

model;
    // 上の階層にある共通数式ファイルを読み込む
    @#include "../polt_model_equations_common.inc"
    
    // Stage 1 では累積ギャップ（履歴効果）は計算に使用しないためゼロ固定
    gamma_H = 0;
...
steady_state_model;
    ...
    // 消費者物価インフレ率の定常状態 (統一ルール pi_F_W_star に準拠)
    pi_H_W = pi_H_W_ss;
...
// ★ 伸縮的価格モデルとするため、価格硬直性パラメータを一時的にゼロとして上書きする
xi_H = 0;
xi_F = 0;
