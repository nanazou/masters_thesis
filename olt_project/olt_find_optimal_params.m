% olt_find_optimal_params.m
% このスクリプトは、OLT政策の最適パラメータを探索し、
% 厚生の表面グラフを出力します。計算の核心部は src/olt_solve_core.m を呼び出します。
...
fprintf('====================================================\n');
fprintf('Starting optimization for OLT Policy...\n');
fprintf('====================================================\n\n');
...
shock_scenario = 'betaa'; % <--- ここを 'beta' または 'a' に書き換えて切り替えます
...
%phi_gap_H_range_coarse   = 0.000000005:0.000000001:0.00000001;
%phi_level_H_range_coarse = 0.000000005:0.000000001:0.00000001;

% --- 4. 粗い探索 (coarse mode) の設定 --- beta final
%phi_gap_H_range_coarse   = 60:5:90;
%phi_level_H_range_coarse = 90:5:110;

% --- 5. 詳細な探索 (fine mode) の設定 --- beta_final
%center_phi_gap_H   = 85; 
%center_phi_level_H = 105; 
%fine_search_radius_gap_H   = 2; 
%fine_search_step_gap_H     = 1; 
%fine_search_radius_level_H = 2; 
%fine_search_step_level_H   = 1;
...
% 接頭辞を統一したモデル名を参照
settings.modnam = 'olt_model';
settings.modnamstar = 'olt_model_zlb';
...
for i = 1:length(phi_level_H_range_final)
    for j = 1:length(phi_gap_H_range_final)
        ...
        % --- [計算核心 olt_solve_core を呼び出す] ---
        try
            % 共通エンジン olt_solve_core.m を呼び出し
            res = olt_solve_core(phi_gap_H_value, phi_level_H_value, settings);
...
% =========================================================================
fprintf('\n--- OLT Final Optimal Policy Results (Shock: %s) ---\n', shock_scenario);
...
h_home = figure('Name', 'OLT: Home Welfare', 'Visible', 'off');
...
line2_H = sprintf('\\phi_{gap}^H=%.10f, \\phi_{level}^H=%.10f, Welfare=%.4f', ...
...
h_foreign = figure('Name', 'OLT: Foreign Welfare', 'Visible', 'off');
...
line2_F = sprintf('\\phi_{gap}^H=%.10f, \\phi_{level}^H=%.10f, Welfare=%.4f', ...
...
h_global = figure('Name', 'OLT: Global Welfare', 'Visible', 'off');
...
line2_G = sprintf('\\phi_{gap}^H=%.10f, \\phi_{level}^H=%.10f, Welfare=%.4f', ...
...
fprintf('\n====================================================\n');
fprintf('OLT Policy optimization has finished for %s.\n', shock_scenario);
