% olt_main.m
% このスクリプトは olt_solve_core.m を呼び出し、特定のパラメータでの結果を詳細に出力します。
...
%% --- 共通シミュレーション設定 ---
% 独自名に変更した統一ファイル名 ('olt_model', 'olt_model_zlb') を参照
...
for s_idx = 1:length(scenarios)
    ...
    % --- [修正箇所] 出力先フォルダの設定 (results > [shock] 直下に変更) ---
    output_root_folder = fullfile(mpath, 'results', current_scen.name); 
    if ~exist(output_root_folder, 'dir'), mkdir(output_root_folder); end

    % --- 【核心】olt_solve_core.m の呼び出し ---
    [welfares, irf_results, Mbase_, oobase_] = olt_solve_core(phi_gap_H_value, phi_level_H_value, settings);

    % 取得した厚生変数を個別に展開 (元のコードの保存ロジックで使用するため)
    ...
    % これにより、以降のeval([var_name, '_pie'])がそのまま動作します
...
% 表示変数のリスト (chi_H および 統計ベース物価指数を補完)
    ...
    % --- graphs フォルダを介さず直下の graphs.pdf に保存 ---
    ...
    for i = 1:plots_per_page:num_vars
        ...
        for j = 1:plots_per_page
            ...
            % --- タイトル生成ロジック (指示に基づき LaTeX 表記を全面的に適用) ---
            if strncmp(var_name, 'eps_', 4)
    ...
    for i = 1:length(var_list)
        var_n = var_list{i};
        if ~strncmp(var_n, 'eps_', 4) 
            vars_to_save{end+1} = [var_n, '_pie']; 
            vars_to_save{end+1} = [var_n, '_lin']; 
        end
    end

    vars_to_save = [vars_to_save, {'welfare_H_with_delta_pie', 'welfare_H_with_delta_lin', ...
                                  'welfare_F_with_delta_pie', 'welfare_F_with_delta_lin', ...
                                  'welfare_H_without_delta_pie', 'welfare_H_without_delta_lin', ...
                                  'welfare_F_without_delta_pie', 'welfare_F_without_delta_lin'}];
