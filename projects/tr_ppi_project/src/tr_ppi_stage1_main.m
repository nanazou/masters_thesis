% src/tr_ppi_stage1_main.m
% Stage 1: Flexible Price Model Simulation (Beta or A Shock)
...
% シミュレーション設定の構築
settings.stage = 1;
settings.modnam = 'tr_ppi_stage1_model'; % src/stage1/tr_ppi_stage1_model.mod を参照
...
% --- [3] 【核心】stage1/tr_ppi_stage1_solve_core.m の呼び出し ---
fprintf('Stage 1 (Flexible Price) シミュレーションを開始します。シナリオ: %s (%s)\n', shock_scenario, shock_folder);
[welfares, irf_results, Mbase_, oobase_] = tr_ppi_stage1_solve_core(phi_gap_H_value, phi_level_H_value, settings);
...
try
    ...
    for i = 1:length(vars_to_save)
        ...
        % スカラー値（Welfare）以外を時系列としてCSVに含める
