% it_cpi_main.m
% このスクリプトは it_cpi_solve_core.m を呼び出し、特定のパラメータでの結果を詳細に出力します。
...
for s_idx = 1:length(scenarios)
    ...
    % --- 【核心】it_cpi_solve_core.m の呼び出し ---
    [welfares, irf_results, Mbase_, oobase_] = it_cpi_solve_core(phi_pi_H_value, settings);
...
% ITモデル用: モデル定義に合わせた変数リスト
    ...
    % --- [修正箇所] graphs フォルダを介さず直下の graphs.pdf に保存 ---
    ...
    % [修正] Output フォルダを介さず直下の data_for_plotting.mat に保存
