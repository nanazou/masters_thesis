% pplt_main.m
% このスクリプトは pplt_solve_core.m を呼び出し、特定のパラメータでの結果を詳細に出力します。
...
% ▼▼▼【計算に使用するパラメータ設定 (PPLT用)】▼▼▼
phi_gap_H_value = 100;
phi_level_H_value = 500;
% ▲▲▲【設定完了】▲▲▲
...
for s_idx = 1:length(scenarios)
    ...
    % --- 【核心】pplt_solve_core.m の呼び出し ---
    [welfares, irf_results, Mbase_, oobase_] = pplt_solve_core(phi_gap_H_value, phi_level_H_value, settings);
