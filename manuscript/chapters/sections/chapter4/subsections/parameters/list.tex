% !TeX root = ../../../../../main.tex
% chapters/sections/chapter4/subsections/parameters/list.tex

\subsection{パラメータ設定の一覧}
\label{sec:chapter4_parameters_list}

% 3列目の幅を自動計算し、右端切れを防止しつつ他のページと余白を統一
\begin{longtable}{@{}p{2.3cm}p{1.8cm}p{\dimexpr\textwidth-4.1cm-4\tabcolsep\relax}@{}}
  \caption{パラメータの設定} \label{tab:chapter4_parameters_list} \\
  \toprule
...
\endlastfoot
    ...
    & 
    定常状態の主観的割引因子。
    年率実質利子率（\(r^*\)）が2\%であることを意味し、
    両国経済が近年の構造的低下に陥る以前の平時の値として設定する。
    \textbf{日本（\(H\)）：}
    \textcite{OkazakiSudo2018}は
    日本の\(r^*\)が1980年代の約4\%から近年は0.3\%まで著しく低下したことを示しており、
    その平時の値として2\%を仮定することは妥当であると考えられる。
    \textbf{米国（\(F\)）：}
    \textcite{HolstonLaubachWilliams2017}らによる世界金融危機以前の\(r^*\)推計値2\%を採用する。
    \\
    \noalign{\smallskip}
    ...
    & 
    定常状態の所得税率。
    World Bankのデータによれば日米の税収のGDP比は10\%程度だが、
    これは社会保障負担を含まないため家計や企業が直面する実際の税負担を網羅していない。
    本稿のモデルではこの\(\tau\)は社会保障も含む広義の所得税率を意味するため
    \textcite{GarinLesterSims2016}などにならい20\%に設定する。
    \\
    \midrule
  ...
  \multicolumn{3}{l}{\textit{米国（\(F\)）の金融政策}} \\
    ...
    \(\varepsilon_t^{i,F}\) & 0 & 外生ショック項（定常状態） \\
    \(\phi_{\pi}^F\) & 1.5 & インフレへの反応係数。\textcite{Taylor1993}が提唱した標準的な値。 \\

  \multicolumn{3}{l}{\textit{日本（\(H\)）の金融政策}} \\
    \noalign{\smallskip}
    \multicolumn{3}{l}{\quad \textit{共通（ショック・補助パラメータ）}} \\
    \(\rho_{\chi}^H\) & 0.9 & 目標パスの持続性 \\
    \(\chi_{ss}^H\) & 1.0 & 目標パスの定常状態値 \\
    \(\rho_{i}^H\) & 0.8 & 利子率平滑化係数 \\
    \(\varepsilon_{ss}^{\chi,H}, \varepsilon_{ss}^{i,H}\) & 0 & 外生ショック項（定常状態） \\
    \(\varepsilon_1^{\chi,H}, \varepsilon_1^{i,H}\) & 0.02 / -0.1 & 外生ショック項（\(\beta\)ショック / \(a\)ショック発生期） \\
    \(\varepsilon_t^{\chi,H}, \varepsilon_t^{i,H}\) (\(t \neq 1\)) & 0 & 外生ショック項（それ以外） \\
    \noalign{\smallskip}
    \multicolumn{3}{l}{\quad \textit{各政策の反応係数}} \\
    \noalign{\smallskip}
