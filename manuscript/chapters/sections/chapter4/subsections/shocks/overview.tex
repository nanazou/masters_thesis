% !TeX root = ../../../../../main.tex
% chapters/sections/chapter4/subsections/shocks/overview.tex

本稿のシミュレーションでは自国家計が貯蓄を好むようになる\(\beta\)ショック、
および生産性が低下する\(a\)ショックを経済すなわち\ref{chap:chapter3}章の動学方程式系に対して与える。
主観的割引因子\(\beta_t^H\)の上釈は家計が我慢強くなり、
...
その後は\(\beta\)および\(a\)の自己回帰過程にしたがって徐々に定常状態へと減衰していく。
\begin{align}
    \label{form:chapter4_shocks_beta_eq}
        \log(\beta_t^H)&=(1-\rho_{\beta}^H)\log(\beta_{ss}^H)+\rho_{\beta}^H\log(\beta_{t-1}^H)+\varepsilon_t^{\beta,H} \\
    \label{form:chapter4_shocks_a_eq}
        \log(a_t^H)&=(1-\rho_{a}^H)\log(a_{ss}^H)+\rho_{a}^H\log(a_{t-1}^H)+\varepsilon_t^{a,H}
\end{align}
自己回帰係数についてはパラメータ設定の表\ref{tab:chapter4_parameters_list}でも示したとおり
\(\rho_{\beta}^H\)および\(\rho_{a}^H\)はともに1より小さい値に設定されており、
