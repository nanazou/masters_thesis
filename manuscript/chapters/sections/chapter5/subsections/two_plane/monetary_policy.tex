...
金融政策の分析上考慮すべき変数は
\(p_t^H,y_t^H,i_t^H,\operatorname{E}_t[\lambda_{t+1}^H],\operatorname{E}_t[p_{t+1}^H]\)
の5つである。
以下では\(i_t^H,\operatorname{E}_t[\lambda_{t+1}^H],\operatorname{E}_t[p_{t+1}^H]\)を仮想的に単独で変化させ
\((y,p)\)平面上の均衡点の動きを観察する。
...
所得の期待限界効用\(\operatorname{E}_t[\lambda_{t+1}^H]\)が低下すると
AD曲線
\eqref{form:chapter5_asad_ad_p_H_eq}
...
\subsubsection{期待生産者物価指数\(\operatorname{E}_t[p_{t+1}^H]\)の上昇}
\label{sec:chapter5_asad_monetary_policy_E_p_H_rise}
期待生産者物価指数\(\operatorname{E}_t[p_{t+1}^H]\)が上昇した場合、
AS曲線
\eqref{form:chapter5_asad_as_p_H_eq}
の右辺第2項が増大するため、\textbf{AS曲線は上へ移動}する。
これにより生産者物価指数\(p_t^H\)は上昇し、生産\(y_t^H\)は下落する。
しかし、このとき期待限界効用\(\operatorname{E}_t[\lambda_{t+1}^H]\)は
所得の限界効用の式
