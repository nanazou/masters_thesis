...
により、総消費指数\(c_t^{h\to W}\)が以下のように定義された。
...
により、自国家計\(h\)が所与の総消費指数\(c_t^{h\to W}\)を保ちながら、
自国財消費指数\(c_t^{h\to H}\)と外国財消費指数\(c_t^{h\to F}\)を調整して名目支出の総和
\(p_t^Hc_t^{h\to H}+p_t^Fc_t^{h\to F}\)を最小化する問題が与えられた。
...
一致することが証明される（式\eqref{chap:appendix_envelope}TODO）。すなわち
\begin{equation}
...
同様に式
\eqref{form:problem_setup_foreign_c_f_W_def}
により、総消費指数\(c_t^{f\to W}\)が以下のように定義された。
これはCES型指数において代替の弾力性パラメータ\(\theta\)が1である特別な場合（コブ＝ダグラス型）に対応する。
\begin{equation}
...
仮に消費指数の単位を「単位」とすると
\begin{equation}
\label{form:chapter3_households_stage1b_foreign_c_f_W_definition_unit_id}
    (\text{単位})^{\alpha^F}\times(\text{単位})^{1-\alpha^F}=\text{単位}^{\alpha^F+1-\alpha^F}=\text{単位}^1
\end{equation}
となり総消費指数\(c_t^{f\to W}\)はその構成要素と同じ次元を持つ。
これにより\(c_t^{f\to W}\)は家計の総体的な消費水準を示す指標として成立する。
\(1/\left((\alpha^F)^{\alpha^F}(1-\alpha^F)^{1-\alpha^F}\right)\)という定数項は
総消費指数\(c_t^{f\to W}\)を調整するための正規化項である。
モデルの式中では消費者物価指数\(p_t^{F\to W*}\)が多用されるため、
その定義式が簡潔になるよう、あらかじめ\(c_t^{f\to W}\)の定義にこの項を入れておくのが一般的である。
この正規化により後続する消費者物価指数\(p_t^{F\to W*}\)の定義式から
定数項\((\alpha^F)^{\alpha^F}(1-\alpha^F)^{1-\alpha^F}\)が消え、
\(p_t^{F\to W*}=(p_t^{F*})^{\alpha^F}(p_t^{H*})^{1-\alpha^F}\)という簡潔な形で表現される。

\paragraph{外国家計の総消費指数\(c_t^{f\to W}\)を最小費用で達成する問題}
\label{sec:chapter3_households_stage1b_foreign_c_f_W_problem}
式
\eqref{form:chapter3_households_problem_setup_home_stage2_max_def}
により、外国家計\(f\)が所与の総消費指数\(c_t^{f\to W}\)を保ちながら、
外国財消費指数\(c_t^{f\to F}\)と自国財消費指数\(c_t^{f\to H}\)を調整して名目支出の総和
\(p_t^{F*}c_t^{f\to F}+p_t^{H*}c_t^{f\to H}\)を最小化する問題が与えられた。
この問題のラグランジアンは以下のように記述される。
\begin{equation}
...
そこで上記ラグランジアンを総消費指数\(c_t^{f\to W}\)で偏微分すると
その値はラグランジュ乗数\(\eta_t^f\)に一致する。
...
生産者物価指数（PPI）\(p_t^H, p_t^F\)に関する一物一価の法則
\eqref{form:chapter3_households_stage1a_ppi_definitions_p_H_lop}および
\eqref{form:chapter3_households_stage1a_ppi_definitions_p_F_lop}
を用いると、異なる通貨建ての消費者物価指数間の関係式が以下のように導出される。
\begin{align}
