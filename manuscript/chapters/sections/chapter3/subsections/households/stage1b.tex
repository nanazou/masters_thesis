...
家計\(h\)が所与の総消費指数\(c_t^{h\to W}\)を達成するために、
自国財消費指数\(c_t^{h\to H}\)と外国財消費指数\(c_t^{h\to F}\)への名目支出の総和
...
そこで上記ラグランジアンを総消費指数\(c_t^{h\to W}\)で偏微分すると、その値はラグランジュ乗数\(\eta_t^h\)に一致する。
...
\end{equation}
このことからラグランジュ乗数\(\eta_t^h\)は総消費指数\(c_t^{h\to W}\)の限界価格と解釈でき、
大雑把にいえば総消費指数\(c_t^{h\to W}\)を追加1単位増やしたときの
名目総支出の増加量を表す。
この費用最小化問題から各財消費指数への需要関数が導出される
...
外国財消費指数\(c_t^{f\to F}\)と自国財消費指数\(c_t^{f\to H}\)への名目支出の総和
...
\end{equation}
このことからラグランジュ乗数\(\eta_t^f\)は総消費指数\(c_t^{f\to W}\)の限界価格と解釈でき、
大雑把にいえば総消費指数\(c_t^{f\to W}\)を追加1単位増やしたときの
名目総支出の増加量を表す。
この費用最小化問題から各財消費指数への需要関数が導出される
（式\eqref{chap:appendix_cost_minimization}TODO）。
\begin{align}
