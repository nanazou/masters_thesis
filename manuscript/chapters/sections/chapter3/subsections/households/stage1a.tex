...
\begin{align}
    \label{form:sec_chapter3_households_stage1a_p_H_def}
        p_t^H&\equiv\mu_t^{h\to H}=\left[\sum_{h'\in H}(p_t^{h'})^{1-\theta^H}\right]^{\frac{1}{1-\theta^H}}\quad\text{（自国財の自国通貨建て生産者物価指数(PPI)）} \\
    \label{form:sec_chapter3_households_stage1a_p_F_star_def}
        p_t^{F*}&\equiv\mu_t^{f\to F}=\left[\sum_{f'\in F}(p_t^{f'*})^{1-\theta^F}\right]^{\frac{1}{1-\theta^F}}\quad\text{（外国財の外国通貨建て生産者物価指数(PPI)）}
\end{align}
