...
により自国財消費指数\(c_t^{h\to H}\)が以下のように定義された。
...
により外国財消費指数\(c_t^{h\to F}\)が以下のように定義された。
\begin{equation}
...
により、家計\(h\)が所与の自国財消費指数\(c_t^{h\to H}\)を保ちながら
家計の財の名目消費の総和\(\sum_{h'\in H}p_t^{h'}c_t^{h\to h'}\)を最小化する問題が与えられた。
...
一致することが証明される（式\eqref{chap:appendix_envelopeTODO}）。すなわち
\begin{equation}
...
により、自国家計\(h\)が所与の外国財消費指数\(c_t^{h\to F}\)を保ちながら
外国家計の財の名目消費の総和\(\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}\)を最小化する問題が与えられた。
この問題のラグランジアンは以下のように記述される。
\begin{equation}
\label{form:chapter3_households_stage1a_home_c_h_F_problem_lagrangian_def}
    \mathcal{L}^{h\to F}=\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}+\mu_t^{h\to F}\left(c_t^{h\to F}-\left[\sum_{f'\in F}(c_t^{h\to f'})^{\frac{\theta^F-1}{\theta^F}}\right]^{\frac{\theta^F}{\theta^F-1}}\right)
\end{equation}
上記ラグランジアンには名目外国財消費\(\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}\)が含まれていることから
単位は自国通貨と考えられる。
また一般にラグランジアンはラグランジュ乗数\(\mu_t^{h\to F}\)を独立変数扱いして各独立変数で偏微分する。
そこで上記ラグランジアンを外国財消費指数\(c_t^{h\to F}\)で偏微分するとその値は
ラグランジュ乗数\(\mu_t^{h\to F}\)に一致する。
さらに均衡においてはこのラグランジアンの外国財消費指数\(c_t^{h\to F}\)による偏微分は
名目外国財消費\(\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}\)の外国財消費指数\(c_t^{h\to F}\)による微分に
一致することが証明される（式\eqref{chap:appendix_envelopeTODO}）。すなわち
\begin{equation}
...
\end{equation}
このことからラグランジュ乗数\(\mu_t^{h\to F}\)は外国財消費指数\(c_t^{h\to F}\)の限界価格と解釈でき、
大雑把にいえば外国財消費指数\(c_t^{h\to F}\)を追加1単位増やしたときの
名目外国財消費\(\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}\)の増加量を表す。
この費用最小化問題から家計\(f'\)が生産する財への需要関数が導出される
（式\eqref{chap:appendix_cost_minimization}TODO）。
\begin{equation}
...
\end{equation}
この需要関数を名目外国財消費\(\sum_{f'\in F}p_t^{f'}c_t^{h\to f'}\)に代入して計算すると
\begin{equation}
...
\end{equation}
という関係が得られる（式\eqref{chap:appendix_cost_minimization}TODO）。
これにより\(\mu_t^{h\to F}\)は外国財消費指数\(c_t^{h\to F}\)の
（限界価格というだけでなく）平均価格であると解釈できる。
さらに具体的に\(\mu_t^{h\to F}\)は個々の財価格のみで構成されることが示される
（式\eqref{chap:appendix_cost_minimization}）。
\begin{equation}
\label{form:chapter3_households_stage1a_home_c_h_F_problem_mu_h_F_eq}
    \mu_t^{h\to F}=\left[\sum_{f'\in F}(p_t^{f'})^{1-\theta^F}\right]^{\frac{1}{1-\theta^F}}
\end{equation}
（ここで\(p_t^{f'}\)は外国財\(f'\)の自国通貨建て価格）
この式からわかるように\(\mu_t^{h\to F}\)はすべての自国家計\(h\)で共通の値をとる。
またこの式によれば\(\mu_t^{h\to F}\)は外国財消費指数\(c_t^{h\to F}\)に依存しないのだから、
外国財消費指数\(c_t^{h\to F}\)の限界価格であった\(\mu_t^{h\to F}\)が
外国財消費指数\(c_t^{h\to F}\)の平均価格ともなったことは当然といえる。
...
式
\eqref{form:problem_setup_foreign_c_f_F_def}
により外国財消費指数\(c_t^{f\to F}\)が以下のように定義された。
...
同様に式
\eqref{form:problem_setup_foreign_c_f_H_def}
により自国財消費指数\(c_t^{f\to H}\)が以下のように定義された。
\begin{equation}
...
により、外国家計\(f\)が所与の外国財消費指数\(c_t^{f\to F}\)を保ちながら
外国家計の財の名目消費の総和\(\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}\)を最小化する問題が与えられた。
この問題のラグランジアンは以下のように記述される。
\begin{equation}
\label{form:chapter3_households_stage1a_foreign_c_f_F_problem_lagrangian_def}
    \mathcal{L}^{f\to F}=\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}+\mu_t^{f\to F}\left(c_t^{f\to F}-\left[\sum_{f'\in F}(c_t^{f\to f'})^{\frac{\theta^F-1}{\theta^F}}\right]^{\frac{\theta^F}{\theta^F-1}}\right)
\end{equation}
上記ラグランジアンには名目外国財消費\(\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}\)が含まれていることから
単位は外国通貨と考えられる。
また一般にラグランジアンはラグランジュ乗数\(\mu_t^{f\to F}\)を独立変数扱いして各独立変数で偏微分する。
そこで上記ラグランジアンを外国財消費指数\(c_t^{f\to F}\)で偏微分するとその値は
ラグランジュ乗数\(\mu_t^{f\to F}\)に一致する。
さらに均衡においてはこのラグランジアンの外国財消費指数\(c_t^{f\to F}\)による偏微分は
名目外国財消費\(\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}\)の外国財消費指数\(c_t^{f\to F}\)による微分に
一致することが証明される（式\eqref{chap:appendix_envelopeTODO}）。すなわち
\begin{equation}
...
\end{equation}
このことからラグランジュ乗数\(\mu_t^{f\to F}\)は外国財消費指数\(c_t^{f\to F}\)の限界価格と解釈でき、
大雑把にいえば外国財消費指数\(c_t^{f\to F}\)を追加1単位増やしたときの
名目外国財消費\(\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}\)の増加量を表す。
この費用最小化問題から家計\(f'\)が生産する財への需要関数が導出される
（式\eqref{chap:appendix_cost_minimization}TODO）。
\begin{equation}
...
\end{equation}
この需要関数を名目外国財消費\(\sum_{f'\in F}p_t^{f'*}c_t^{f\to f'}\)に代入して計算すると
\begin{equation}
...
\end{equation}
という関係が得られる（式\eqref{chap:appendix_cost_minimization}TODO）。
これにより\(\mu_t^{f\to F}\)は外国財消費指数\(c_t^{f\to F}\)の
（限界価格というだけでなく）平均価格であると解釈できる。
さらに具体的に\(\mu_t^{f\to F}\)は個々の財価格のみで構成されることが示される
（式\eqref{chap:appendix_cost_minimization}）。
\begin{equation}
\label{form:chapter3_households_stage1a_foreign_c_f_F_problem_mu_f_F_eq}
    \mu_t^{f\to F}=\left[\sum_{f'\in F}(p_t^{f'*})^{1-\theta^F}\right]^{\frac{1}{1-\theta^F}}
\end{equation}
（ここで\(p_t^{f'*}\)は外国財\(f'\)の外国通貨建て価格）
この式からわかるように\(\mu_t^{f\to F}\)はすべての外国家計\(f\)で共通の値をとる。
またこの式によれば\(\mu_t^{f\to F}\)は外国財消費指数\(c_t^{f\to F}\)に依存しないのだから、
外国財消費指数\(c_t^{f\to F}\)の限界価格であった\(\mu_t^{f\to F}\)が
外国財消費指数\(c_t^{f\to F}\)の平均価格ともなったことは当然といえる。
...
により、家計\(f\)が所与の自国財消費指数\(c_t^{f\to H}\)を保ちながら
家計の財の名目消費の総和\(\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}\)を最小化する問題が与えられた。
この問題のラグランジアンは以下のように記述される。
\begin{equation}
\label{form:chapter3_households_stage1a_foreign_c_f_H_problem_lagrangian_def}
    \mathcal{L}^{f\to H}=\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}+\mu_t^{f\to H}\left(c_t^{f\to H}-\left[\sum_{h'\in H}(c_t^{f\to h'})^{\frac{\theta^H-1}{\theta^H}}\right]^{\frac{\theta^H}{\theta^H-1}}\right)
\end{equation}
上記ラグランジアンには名目自国財消費\(\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}\)が含まれていることから
単位は外国通貨と考えられる。
また一般にラグランジアンはラグランジュ乗数\(\mu_t^{f\to H}\)を独立変数扱いして各独立変数で偏微分する。
そこで上記ラグランジアンを自国財消費指数\(c_t^{f\to H}\)で偏微分するとその値は
ラグランジュ乗数\(\mu_t^{f\to H}\)に一致する。
さらに均衡においてはこのラグランジアンの自国財消費指数\(c_t^{f\to H}\)による偏微分は
名目自国財消費\(\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}\)の自国財消費指数\(c_t^{f\to H}\)による微分に
一致することが証明される（式\eqref{chap:appendix_envelopeTODO}）。すなわち
\begin{equation}
...
\end{equation}
このことからラグランジュ乗数\(\mu_t^{f\to H}\)は自国財消費指数\(c_t^{f\to H}\)の限界価格と解釈でき、
大雑把にいえば自国財消費指数\(c_t^{f\to H}\)を追加1単位増やしたときの
名目自国財消費\(\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}\)の増加量を表す。
この費用最小化問題から家計\(h'\)が生産する財への需要関数が導出される
（式\eqref{chap:appendix_cost_minimization}TODO）。
\begin{equation}
...
\end{equation}
この需要関数を名目自国財消費\(\sum_{h'\in H}p_t^{h'*}c_t^{f\to h'}\)に代入して計算すると
\begin{equation}
...
\end{equation}
という関係が得られる（式\eqref{chap:appendix_cost_minimization}TODO）。
これにより\(\mu_t^{f\to H}\)は自国財消費指数\(c_t^{f\to H}\)の
（限界価格というだけでなく）平均価格であると解釈できる。
さらに具体的に\(\mu_t^{f\to H}\)は個々の財価格のみで構成されることが示される
（式\eqref{chap:appendix_cost_minimization}）。
\begin{equation}
\label{form:chapter3_households_stage1a_foreign_c_f_H_problem_mu_f_H_eq}
    \mu_t^{f\to H}=\left[\sum_{h'\in H}(p_t^{h'*})^{1-\theta^H}\right]^{\frac{1}{1-\theta^H}}
\end{equation}
（ここで\(p_t^{h'*}\)は自国財\(h'\)の外国通貨建て価格）
この式からわかるように\(\mu_t^{f\to H}\)はすべての外国家計\(f\)で共通の値をとる。
またこの式によれば\(\mu_t^{f\to H}\)は自国財消費指数\(c_t^{f\to H}\)に依存しないのだから、
自国財消費指数\(c_t^{f\to H}\)の限界価格であった\(\mu_t^{f\to H}\)が
自国財消費指数\(c_t^{f\to H}\)の平均価格ともなったことは当然といえる。

\subsubsection{生産者物価指数（PPI）\(p_t^H, p_t^{F*}\)の定義}
\label{sec:chapter3_households_stage1a_ppi_definitions}
一物一価の法則
\eqref{form:chapter3_assumptions_lop_home_eq}および
\eqref{form:chapter3_assumptions_lop_foreign_eq}を用いると
...
\begin{align}
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_H_bar_def}
        \bar{p}_t^H&\equiv\left[\frac{1}{N}\sum_{h'\in H}(p_t^{h'})^{1-\theta^H}\right]^{\frac{1}{1-\theta^H}} \\
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_F_star_bar_def}   
        \bar{p}_t^{F*}&\equiv\left[\frac{1}{M}\sum_{f'\in F}(p_t^{f'*})^{1-\theta^F}\right]^{\frac{1}{1-\theta^F}}
\end{align}
またこれらは右辺に通常の生産者物価指数（PPI）の定義式
\eqref{form:chapter3_households_stage1a_p_H_def}および
\eqref{form:chapter3_households_stage1a_p_F_star_def}
を代入することにより、以下のように書き換えられる。
\begin{align}
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_H_bar_eq}
        \bar{p}_t^H&=N^{-\frac{1}{1-\theta^H}}p_t^H \\
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_F_star_bar_eq}   
        \bar{p}_t^{F*}&=M^{-\frac{1}{1-\theta^F}}p_t^{F*}
\end{align}
するとこれらについても一物一価の法則が成立する。
\begin{align}
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_H_bar_lop_eq}
        \bar{p}_t^H&=e_t^{/*}\bar{p}_t^{H*}\\
    \label{form:chapter3_households_stage1a_normalized_ppi_definitions_p_F_bar_lop_ea}
