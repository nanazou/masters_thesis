...
そして価格改定の機会を得た家計\(h\)は
問題\ref{form:chapter3_households_stage2_home_utility_def}において価格\(p_t^h\)を最適化する。
また\ref{sec:chapter3_assumptions_producer_optimization_home_calculation}より
価格\(p_t^h\)の関数とみなすのは価格\(p_t^h\)のほか労働\(l_t^h\)と生産\(y_t^h\)のみである。
さらに\(t\)期で設定した価格\(p_t^h\)が\(k\)期先まで維持される確率は\((\xi^H)^k\)となるため
この問題は以下の問題と同値となる（\ref{}節TODO）。
\begin{equation}
\label{form:chapter3_producers_problem_setup_home_lagrangian_def}
    \begin{aligned}
...
そして価格改定の機会を得た家計\(f\)は
問題\ref{form:chapter3_households_stage2_foreign_utility_def}において価格\(p_t^{f*}\)を最適化する。
また\ref{sec:chapter3_assumptions_producer_optimization_foreign_calculation}より
価格\(p_t^{f*}\)の関数とみなすのは価格\(p_t^{f*}\)のほか労働\(l_t^f\)と生産\(y_t^f\)のみである。
さらに\(t\)期で設定した価格\(p_t^{f*}\)が\(k\)期先まで維持される確率は\((\xi^F)^k\)となるため
この問題は以下の問題と同値となる（\ref{}節TODO）。
\begin{equation}
\label{form:chapter3_producers_problem_setup_foreign_lagrangian_def}
    \begin{aligned}
