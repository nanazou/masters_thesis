...
\begin{equation}
\label{form:chapter3_representative_household_euler_contingent_home_representative_lambda_H_i_H_forward_eq}
    \lambda_t^H
    =\operatorname{E}_t\left[\left(\prod_{j=0}^{k-1}\beta_{t+j}^H(1+i_{t+j}^H)\right)\lambda_{t+k}^H\right]
\end{equation}
...
\begin{equation}
\label{form:chapter3_representative_household_euler_contingent_home_representative_lambda_H_i_H_lim_eq}
    \begin{aligned}
        \lambda_t^H
...
を適用する操作を\(k\)期先まで繰り返すと以下が得られる。
\begin{equation}
\label{form:chapter3_representative_household_euler_contingent_foreign_representative_lambda_F_slash_star_i_F_forward_eq}
    \lambda_t^{F/*}
    =\operatorname{E}_t\left[\left(\prod_{j=0}^{k-1}\beta_{t+j}^F(1+i_{t+j}^F)\right)\lambda_{t+k}^{F/*}\right]
\end{equation}
本稿が使用するDynareによるシミュレーションにおいてはショックを受けた経済は
唯一の安定パス（サドルパス）をたどり新たな定常状態へ収束する。ただし実質変数は初期定常状態に収束する。
上式の両辺について極限\(\lim_{k\to\infty}\)をとると以下の
...
\begin{equation}
\label{form:chapter3_representative_household_euler_contingent_foreign_representative_lambda_F_slash_star_i_F_lim_eq}
    \begin{aligned}
        \lambda_t^{F/*}
        &=(\lim_{k\to\infty}\lambda_{t+k}^{F/*})\operatorname{E}_t\left[\prod_{j=0}^{\infty}\beta_{t+j}^F(1+i_{t+j}^F)\right] \\
        &=(\lim_{k\to\infty}\frac{1}{p_{t+k}^{F\to W*}c_{t+k}^{F\to W}})\operatorname{E}_t\left[\prod_{j=0}^{\infty}\beta_{t+j}^F(1+i_{t+j}^F)\right] \\
        &=(\frac{1}{(\lim_{k\to\infty}p_{t+k}^{F\to W*})(\lim_{k\to\infty}c_{t+k}^{F\to W})})\operatorname{E}_t\left[\prod_{j=0}^{\infty}\beta_{t+j}^F(1+i_{t+j}^F)\right] \\
        &=(\frac{1}{(\lim_{k\to\infty}p_{t+k}^{F\to W*})c_{ss}^{F\to W}})\operatorname{E}_t\left[\prod_{j=0}^{\infty}\beta_{t+j}^F(1+i_{t+j}^F)\right]
    \end{aligned}
\end{equation}