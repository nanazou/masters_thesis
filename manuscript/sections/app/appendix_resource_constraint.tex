...
\label{eq:appendix_resource_constraint_theorem7_main}
\end{equation}
...
\end{aligned}
\label{eq:appendix_resource_constraint_sub1_aggregate_sum}
\end{equation}
...
\begin{equation}
\sum_{h\in H}d_t^{h\to H}=0\quad\text{and}\quad\sum_{h\in H}\sum_{j'}q_{t+1}(j')d_{t+1}^{h\to H}(j')=0
\label{eq:appendix_resource_constraint_sub2_bond_clearing}
\end{equation}
...
\label{eq:appendix_resource_constraint_sub2_gov_budget}
\end{equation}
...
\label{eq:appendix_resource_constraint_sub3_remaining_terms}
\end{equation}
...
\label{eq:appendix_resource_constraint_sub3_consumption_agg}
\end{equation}
対外純資産（自国通貨建て）については、単純な総和として定義される。
\begin{equation}
\sum_{h\in H}b_{t+1}^{h\to F}\equiv B_{t+1}^H
\label{eq:appendix_resource_constraint_sub3_nfa_def}
\end{equation}
...
\begin{equation}
p_t^{H\to W}C_t^{H\to W}+B_{t+1}^H=p_t^HY_t^H+(1+i_{t-1}^F)\frac{e_t}{e_{t-1}}B_t^H
\label{eq:appendix_resource_constraint_sub4_final}
\end{equation}