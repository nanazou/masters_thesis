...
本付録では、本稿で導出された家計の最適計画が時間整合的であること、
...
\label{sec:appendix_time_consistency_sub1}
...
\end{aligned}
\label{eq:appendix_time_consistency_sub1_lagrangian_s}
\end{equation}
将来の任意の時点\(t(>s)\)における総消費指数\(c_t^{h\to W}\)に関する一階の条件は、
\begin{equation}
\frac{\partial\mathcal{L}_s^h}{\partial c_t^{h\to W}}=\operatorname{E}_s\left[D_{s,t}\left(\frac{1}{c_t^{h\to W}}-\lambda_t^hp_t^{H\to W}\right)\right]=0
\label{eq:appendix_time_consistency_sub1_foc_s}
\end{equation}
...
\end{aligned}
\label{eq:appendix_time_consistency_sub1_lagrangian_t}
\end{equation}
このとき、現時点である時点\(t\)の消費\(c_t^{h\to W}\)に関する一階の条件は、
\begin{equation}
\frac{\partial\mathcal{L}_t^h}{\partial c_t^{h\to W}}=\frac{1}{c_t^{h\to W}}-\lambda_t^hp_t^{H\to W}=0
\label{eq:appendix_time_consistency_sub1_foc_t}
\end{equation}
となり、時点\(s\)で計画した関係式と全く同一の形式が導出される。

\section{価格設定に関する時間整合性}
\label{sec:appendix_time_consistency_sub2}
...
\begin{equation}
\operatorname{E}_t\sum_{k=0}^{\infty}(\xi^H)^kD_{t,t+k}\left[\frac{\partial\text{Utility}_{t+k}}{\partial p_t^h}\right]=0
\label{eq:appendix_time_consistency_sub2_pricing_foc_t}
\end{equation}
...
\begin{equation}
\operatorname{E}_{t+n}\sum_{m=0}^{\infty}(\xi^H)^mD_{t+n,t+n+m}\left[\frac{\partial\text{Utility}_{t+n+m}}{\partial p_t^h}\right]=0
\label{eq:appendix_time_consistency_sub2_pricing_foc_tn}
\end{equation}
となる。指数型割引の性質から、時点\(t\)における総和式は、時点\(t+n\)以降の総和式を包含した構造（\((\xi^H)^nD_{t,t+n}\)という定数倍の差のみ）になっている。したがって、新たなショックが発生しない限り、当初の最適価格\(p_t^h\)は将来のどの時点においても依然として最適条件を満たし続ける。

\section{結論}
\label{sec:appendix_time_consistency_conclusion}
