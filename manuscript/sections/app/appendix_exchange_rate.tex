...
\end{align*}
ここで、右辺の期待値の中身は微小変動同士の積（ \( \hat{x} \times \hat{y} \) のオーダー ）となっている。 1 次近似の枠組みでは、 1 次の微小量同士の積は 2 次の微小量となるため、無視することができる（ \( \approx 0 \) ）。したがって、以下の近似が成立する。
\begin{equation}
...
\ref{sec:appendix_insulation_effect_modified_uip}節で確認したとおり、
...
前節で導出した再帰式 \eqref{eq:appendix_exchange_rate_lambda_recursive} を将来に向かって逐次代入することで、現在の比率 \( \Lambda_t \) を決定する。
...
\( \Lambda_t \) の定義式\eqref{eq:appendix_exchange_rate_lambda_def}を変形した 
 \( \lambda_t^{H/*} = \Lambda_t \lambda_t^{F/*} \) を代入すると以下が得られる。
\begin{equation}
    e_t^{/*} = \Lambda_t \times \frac{\lambda_t^{F/*}}{\lambda_t^H}
    \label{eq:appendix_exchange_mu_ratio}
\end{equation}

ここで式\eqref{eq:appendix_exchange_mu_ratio}に
...
\begin{equation}
    ...
    \label{eq:appendix_exchange_euler_ratio}
\end{equation}

一方で式 \eqref{eq:appendix_exchange_mu_ratio} に
