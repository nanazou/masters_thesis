...
\subsection{財政政策}
\label{sec:model_policies_fiscal_policy}
\paragraph{自国の財政政策}\label{sec:model_policies_fiscal_policy_home}
...
\label{eq:model_policies_tax_rule_home}
\end{equation}
政府の予算制約式は\(Nt_t^H=\sum_{h\in H}\tau_t^Hp_t^hy_t^h\)である。
ここで付録A.4（定理4）において証明されるように総名目所得は以下の関係式で表すことができる。
\begin{equation}
\sum_{h\in H}p_t^hy_t^h=p_t^HY_t^H
\label{eq:model_policies_nominal_income_identity}
\end{equation}
これらより集計レベルの予算制約式は以下のように決定される。
\begin{equation}
Nt_t^H=\tau_t^Hp_t^HY_t^H
\label{eq:model_policies_gov_budget_aggregate}
\end{equation}
これより一人当たり移転額は以下のように決定される。
\begin{equation}
t_t^H=\tau_t^H\frac{p_t^HY_t^H}{N}
\label{eq:model_policies_transfer_home}
\end{equation}



\paragraph{外国の財政政策}\label{sec:model_policies_fiscal_policy_foreign}
同様に外国政府の税率\(\tau_t^F\)は外生的な確率過程に従い、一人当たり移転額は以下のように決定される。
\begin{equation}
t_t^{F*}=\tau_t^F\frac{p_t^{F*}Y_t^F}{M}
\label{eq:model_policies_transfer_foreign}
\end{equation}
\begin{equation}
\log(\tau_t^F)=(1-\rho_{\tau}^F)\log(\tau_{ss}^{F*})+\rho_{\tau}^F\log(\tau_{t-1}^F)+\varepsilon_t^{\tau,F}
\label{eq:model_policies_tax_rule_foreign}
\end{equation}
...
\label{eq:model_policies_pi_hat_derivation}
\end{align}
と計算される。
...
\label{eq:model_policies_pi_hat_dynamics_definition}
\end{equation}
...
\label{eq:model_policies_monetary_policy_it_home}
\end{equation}
...
\begin{align}
i_t^H&=i_{ss}^H+\phi_{gap}(\log(\bar{p}_t^Hy_t^H)-\log(\chi_t^H))+\phi_{level}\gamma_t^H \label{eq:model_policies_mp_ngdplt_interest_rate} \\
\gamma_t^H&=\gamma_{t-1}^H+(\log(\bar{p}_{t-1}^Hy_{t-1}^H)-\log(\chi_{t-1}^H)) \label{eq:model_policies_mp_ngdplt_accumulation}
\end{align}
...
\label{eq:model_policies_mp_ngdplt_target_path}
\end{equation}

\paragraph{3.名目消費水準目標(NCLT)}\label{sec:model_policies_monetary_policy_nclt}
利子率は一人当たりの名目総消費\(p_t^{H\to W}c_t^{H\to W}\)の
目標パス\(\chi_t^H\)からの対数乖離（\(\log(p_t^{H\to W}c_t^{H\to W})-\log(\chi_t^H)\)）
に反応する。
\begin{align}
i_t^H&=i_{ss}^H+\phi_{gap}(\log(p_t^{H\to W}c_t^{H\to W})-\log(\chi_t^H))+\phi_{level}\gamma_t^H \label{eq:model_policies_mp_nclt_interest_rate} \\
\gamma_t^H&=\gamma_{t-1}^H+(\log(p_{t-1}^{H\to W}c_{t-1}^{H\to W})-\log(\chi_{t-1}^H)) \label{eq:model_policies_mp_nclt_accumulation}
\end{align}
目標パス\(\chi_t^H\)は名目GDP水準目標と同様の確率過程に従う。
\begin{equation}
\log(\chi_t^H)=(1-\rho_{\chi}^H)\log(\chi_{ss}^H)+\rho_{\chi}^H\log(\chi_{t-1}^H)+\varepsilon_t^{\chi,H}
\label{eq:model_policies_mp_nclt_target_path}
\end{equation}

\paragraph{外国の金融政策}\label{sec:model_policies_monetary_policy_foreign}
...
\label{eq:model_policies_monetary_policy_it_foreign}
\end{equation}
