...
\label{sec:model_risk_sharing_lambda_commonality}
...
\begin{equation}
\lambda_t^hq_{t,t+1}=\beta_t^H\pi\lambda_{t+1}^h\quad,\quad\lambda_t^{h'}q_{t,t+1}=\beta_t^H\pi\lambda_{t+1}^{h'}
\label{eq:model_risk_sharing_lambda_commonality_foc_contingent_bonds}
\end{equation}
...
\label{eq:model_risk_sharing_lambda_commonality_marginal_utility_ratio}
\end{equation}
...
この条件を式\eqref{eq:model_risk_sharing_lambda_commonality_marginal_utility_ratio}に適用することで\(k=1\)が特定される。
...
\label{eq:model_risk_sharing_lambda_commonality_commonality_result}
\end{equation}

\subsection{総消費指数\(c_t^{h\to W}\)の共通化}
\label{sec:model_risk_sharing_consumption_commonality}
所得の限界効用がすべての家計で共通であるという上記の結果を前節で導出した消費に関する
一階の条件\eqref{eq:model_households_foc_consumption}に適用する。

\begin{equation}
\lambda_t^h=\frac{1}{p_t^{H\to W}c_t^{h\to W}}\tag{\ref{eq:model_households_foc_consumption}}
\label{eq:model_risk_sharing_consumption_commonality_foc_restated}
\end{equation}
...
\label{eq:model_risk_sharing_consumption_commonality_result}
\end{equation}
...
つまり消費は共通化されるが貯蓄や資産保有額の経路は家計ごとに個別化されるのである。