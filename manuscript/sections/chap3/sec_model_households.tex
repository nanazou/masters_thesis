...
\label{eq:model_households_optimization_home_utility_function}
\end{equation}
...
\end{aligned}
\label{eq:model_households_optimization_home_budget_equation}
\end{equation}
...
\label{eq:model_households_optimization_foreign_utility_function}
\end{equation}
...
\end{aligned}
\label{eq:model_households_optimization_foreign_budget_equation}
\end{equation}
...
\label{eq:model_households_indices_ces_form}
\end{equation}
...
\label{eq:model_households_indices_ces_units}
\end{equation}
...
\label{eq:model_households_indices_mrs}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_domestic_def}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_foreign_def}
\end{equation}

\paragraph{費用最小化と需要関数（自国家計\(\to\)自国財消費指数）}\label{sec:model_households_indices_aggregation_stage1a_costmin_h_h}
...
\label{eq:model_households_indices_aggregation_stage1a_costmin_h_h_lagrangian}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_costmin_h_h_demand}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_costmin_h_h_ppi}
\end{equation}
この式からわかるように\(\mu_t^{h\to H}\)は個々の財価格\(p_t^{h'}\)のみで決定され、
家計\(h\)自身の選択（\(c_t^{h\to h'}\)など）には依存しない。
したがって\(\mu_t^{h\to H}\)はすべての自国家計で共通の値をとる。

\paragraph{他の物価指数（\(\mu\)）の導出}\label{sec:model_households_indices_aggregation_stage1a_ppi_other}
...
\begin{enumerate}
    ...
    \label{eq:model_households_indices_aggregation_stage1a_ppi_h_f}
    \end{equation}
    ...
    \label{eq:model_households_indices_aggregation_stage1a_ppi_f_f}
    \end{equation}
    ...
    \label{eq:model_households_indices_aggregation_stage1a_ppi_f_h}
    \end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_ppi_relation_h}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1a_ppi_relation_f}
\end{equation}

したがって自国通貨建ての生産者物価指数(PPI)は
外国通貨建ての生産者物価指数(PPI)に名目為替レートを乗じたものと一致する。

\paragraph{記法の簡略化とマクロ物価指数}\label{sec:model_households_indices_aggregation_notation}
今後の記述を簡潔にするため、家計が直面する2つの基本的な生産者物価指数(PPI)を
以下のように\(p_t^H\)と\(p_t^{F*}\)で表記する。
\begin{align}
p_t^H&\equiv\mu_t^{h\to H}\quad\text{（自国財の自国通貨建て生産者物価指数(PPI)）}
\label{eq:model_households_indices_aggregation_stage1a_def_ph} \\
p_t^{F*}&\equiv\mu_t^{f\to F}\quad\text{（外国財の外国通貨建て生産者物価指数(PPI)）}
\label{eq:model_households_indices_aggregation_stage1a_def_pfstar}
\end{align}
...
\label{eq:model_households_indices_aggregation_stage1a_def_pbar_h} \\
\bar{p}_t^{F*}&\equiv\left[\frac{1}{M}\sum_{f'\in F}(p_t^{f'*})^{1-\theta^F}\right]^{\frac{1}{1-\theta^F}}=M^{-\frac{1}{1-\theta^F}}p_t^{F*}
\label{eq:model_households_indices_aggregation_stage1a_def_pbar_fstar}
\end{align}

\paragraph{費用最小化に対応する需要関数}\label{sec:model_households_indices_aggregation_stage1a_demand_summary}
...
\begin{itemize}
    ...
    \label{eq:model_households_indices_aggregation_stage1a_demand_h_h}
    \end{equation}
    \item 自国家計による外国財\(f'\)への需要:
    \begin{equation}
    c_t^{h\to f'}=\left(\frac{p_t^{f'}}{p_t^F}\right)^{-\theta^F}c_t^{h\to F}
    \label{eq:model_households_indices_aggregation_stage1a_demand_h_f}
    \end{equation}
    \item 外国家計による外国財\(f'\)への需要:
    \begin{equation}
    c_t^{f\to f'}=\left(\frac{p_t^{f'*}}{p_t^{F*}}\right)^{-\theta^F}c_t^{f\to F}\label{eq:model_households_indices_aggregation_stage1a_demand_f_f}
    \end{equation}
    \item 外国家計による自国財\(h'\)への需要:
    \begin{equation}
    c_t^{f\to h'}=\left(\frac{p_t^{h'*}}{p_t^{H*}}\right)^{-\theta^H}c_t^{f\to H}\label{eq:model_households_indices_aggregation_stage1a_demand_f_h}
    \end{equation}
\end{itemize}

また各最小化問題における名目消費と消費指数の関係は以下の通りである。
\begin{itemize}
    \item \begin{equation}\sum_{h'}p_t^{h'}c_t^{h\to h'}=p_t^Hc_t^{h\to H}\label{eq:model_households_indices_aggregation_stage1a_nom_c_h_h}\end{equation}
    \item \begin{equation}\sum_{f'}p_t^{f'}c_t^{h\to f'}=p_t^Fc_t^{h\to F}\label{eq:model_households_indices_aggregation_stage1a_nom_c_h_f}\end{equation}
    \item \begin{equation}\sum_{f'}p_t^{f'*}c_t^{f\to f'}=p_t^{F*}c_t^{f\to F}\label{eq:model_households_indices_aggregation_stage1a_nom_c_f_f}\end{equation}
    \item \begin{equation}\sum_{h'}p_t^{h'*}c_t^{f\to h'}=p_t^{H*}c_t^{f\to H}\label{eq:model_households_indices_aggregation_stage1a_nom_c_f_h}\end{equation}
\end{itemize}
...
\paragraph{自国家計の費用最小化}\label{sec:model_households_indices_aggregation_stage1b_costmin_h_w}
総消費指数は以下のように定義される。
\begin{equation}
c_t^{h\to W}\equiv\frac{(c_t^{h\to H})^{\alpha^H}(c_t^{h\to F})^{1-\alpha^H}}{(\alpha^H)^{\alpha^H}(1-\alpha^H)^{1-\alpha^H}}
\label{eq:model_households_indices_aggregation_stage1b_world_def}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_units}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_lagrangian_world}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_demand_h_pre} \\
p_t^Fc_t^{h\to F}&=(1-\alpha^H)\eta_t^hc_t^{h\to W}
\label{eq:model_households_indices_aggregation_stage1b_demand_f_pre}
\end{align}
...
\label{eq:model_households_indices_aggregation_stage1b_nom_total_pre}
\end{equation}
という関係が得られる。
...
\label{eq:model_households_indices_aggregation_stage1b_marginal_p}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_cpi_h_def}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_pbar_cpi_h_def}
\end{equation}
...
\eqref{eq:model_households_indices_aggregation_stage1b_demand_h_pre}と
\eqref{eq:model_households_indices_aggregation_stage1b_demand_f_pre}に代入し、
\(c_t^{h\to H}\)と\(c_t^{h\to F}\)について解くことで最終的な需要関数が得られる。
\begin{align}
c_t^{h\to H}&=\alpha^H\frac{p_t^{H\to W}}{p_t^H}c_t^{h\to W}
\label{eq:model_households_indices_aggregation_stage1b_demand_h_final} \\
c_t^{h\to F}&=(1-\alpha^H)\frac{p_t^{H\to W}}{p_t^F}c_t^{h\to W}
\label{eq:model_households_indices_aggregation_stage1b_demand_f_final}
\end{align}

ここで定義上は集計の際の重みとして導入されたパラメータ\(\alpha^H\)が
家計の最適化の結果として支出シェアという経済学的な意味を持つことを示す。
式\eqref{eq:model_households_indices_aggregation_stage1b_demand_h_pre}は
自国財消費指数への名目支出額\(p_t^Hc_t^{h\to H}\)が総消費指数への名目総支出額
\(\eta_t^hc_t^{h\to W}=p_t^{H\to W}c_t^{h\to W}\)の\(\alpha^H\)倍に等しくなることを
直接的に示している。
同様に式\eqref{eq:model_households_indices_aggregation_stage1b_demand_f_pre}は
...
\paragraph{外国家計の費用最小化}\label{sec:model_households_indices_aggregation_stage1b_costmin_f_w}
...
\label{eq:model_households_indices_aggregation_stage1b_cpi_f_def}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_pbar_cpi_f_def}
\end{equation}
...
\label{eq:model_households_indices_aggregation_stage1b_demand_f_f_final} \\
c_t^{f\to H}&=(1-\alpha^F) \frac{p_t^{F\to W*}}{p_t^{H*}}c_t^{f\to W}
\label{eq:model_households_indices_aggregation_stage1b_demand_h_f_final}
\end{align}
名目総消費と指数の間には\(p_t^{F*}c_t^{f\to F}+p_t^{H*}c_t^{f\to H}=p_t^{F\to W*}c_t^{f\to W}\)という関係が成り立つ。

\subsubsection{最適化の一階条件（期をまたぐ問題）}
\label{sec:model_households_foc}
ここからは2段階最適化の第2段階、すなわち期をまたぐ最適化問題を扱う。
第一段階である期内費用最小化の結果、
家計の名目総支出\(\sum_{h'}p_t^{h'}c_t^{h\to h'}+\sum_fp_t^fc_t^{h\to f}\)は
最終財の物価指数\(p_t^{H\to W}\)と消費指数\(c_t^{h\to W}\)の積で簡潔に表現できることがわかる
（証明は付録Aに譲る）。
\begin{equation}
\sum_{h'}p_t^{h'}c_t^{h\to h'}+\sum_fp_t^fc_t^{h\to f}=p_t^{H\to W}c_t^{h\to W}
\label{eq:model_households_foc_nom_total_id}
\end{equation}
...
\label{eq:model_households_foc_world_index_eq}
\end{equation}
...
\label{eq:model_households_foc_h_bond}
\end{equation}
...
\label{eq:model_households_foc_h_uip}
\end{equation}
ここで\(i_t^F\)は外国の無リスク名目金利である。

\paragraph{外国家計のFOC}\label{sec:model_households_foc_f}
