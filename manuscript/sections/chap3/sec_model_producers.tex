...
\subsection{価格の最適化}
\label{sec:model_optimization_price}
本節では家計の価格の最適化を考えていく。
...
\begin{equation}
y_t^h = a_t^H l_t^h
\label{eq:home_production_function}
\end{equation}
同様に外国の家計\(f\)の生産関数は以下のように定義される。
\begin{equation}
y_t^f = a_t^F l_t^f
\label{eq:home_production_function_foreign}
\end{equation}
...
\end{equation}
この均衡条件に家計\(h\)の財の
需要関数\eqref{eq:demand_h_h_prime}と\eqref{eq:demand_f_h_prime}を代入し
...
\begin{enumerate}
   ...
   \(l_t^h\)について解き総労働の定義に代入する。
    ...
    \end{align*}
    \item 価格の非効率性を表す価格分散\(\Delta_t^H\)を以下のように定義する。
    \begin{equation}
    ...
    \label{eq:Home_labor_demand_dispersion_relation}
    \end{equation}
\end{enumerate}
ここでマクロの集計生産指数\(Y_t^H\)を何らかの方法で定義することにより
\begin{equation}
Y_t^H = \frac{a_t^H L_t^H}{\Delta_t^H}
\label{eq:Home_aggregate_production_function}
\end{equation}
という簡潔な集計生産関数の式を成立させられないかと考える。
この目標とする式 \eqref{eq:Home_aggregate_production_function} を\(L_t^H\)について解くと
\(L_t^H = (Y_t^H / a_t^H) \Delta_t^H\)となる。
この式とステップ4で導出した関係式 \eqref{eq:Home_labor_demand_dispersion_relation} を比較すると、
...
\subsection{価格の設定規則}
\label{sec:model_price_setting}
価格の設定規則は \textcite{Calvo1983} に従う。
