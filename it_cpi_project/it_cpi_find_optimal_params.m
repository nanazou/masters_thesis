% it_cpi_find_optimal_params.m
% このスクリプトは、正規化されたCPIベースのIT政策の最適パラメータを探索し、
% 厚生の2次元線グラフを出力します。計算の核心部は src/it_cpi_solve_core.m を呼び出します。
% 探索対象: 自国消費者物価インフレ反応係数 (phi_pi_H)
...
fprintf('====================================================\n');
fprintf('Starting optimization for IT_CPI Policy...\n');
fprintf('====================================================\n\n');
...
% --- 2. 探索範囲の設定 (1次元ラインサーチ用) --- beta final
% 開始する数字 : 間隔 : 終わりの数字
%phi_pi_H_range = 2 : 10 : 300; 

% --- 2. 探索範囲の設定 (1次元ラインサーチ用) --- a final
% 開始する数字 : 間隔 : 終わりの数字
...
% 独自名に変更した統一ファイル名 (it_cpi_model) を参照
settings.modnam = 'it_cpi_model';
settings.modnamstar = 'it_cpi_model_zlb';
...
% 定常状態値をベースワークスペースへ橋渡し (命名規則: ssは最後)
assignin('base', 'pi_H_W_ss', 1.0);

% =========================================================================
%% --- Main Calculation Loop (1D Line Search) ---
% =========================================================================
fprintf('--- Starting 1D Line search for phi_pi_H (Normalized CPI) for %s shock ---\n', shock_scenario);
...
for i = 1:total_iterations
    ...
    % --- [計算核心 src/it_cpi_solve_core.m を関数として呼び出す] ---
    try
        % 接頭辞を統一したコアエンジンを呼び出し
        res = it_cpi_solve_core(phi_pi_H_value, settings);
...
% =========================================================================
fprintf('\n--- IT_CPI Final Optimal Policy Results (Shock: %s) ---\n', shock_scenario);
...
h_home = figure('Name', 'IT_CPI: Home Welfare', 'Visible', 'off');
plot(phi_pi_H_range, results_welfare_H, 'b-', 'LineWidth', 2);
hold on;
plot(final_results.opt_phi_pi_H_H, final_results.max_welfare_H, 'r*', 'MarkerSize', 12, 'LineWidth', 1.5);
grid on;
xlabel('\phi_{\pi}^{H} (Normalized CPI)'); ylabel('Welfare');
line1_H = sprintf('IT_{CPI}: Optimal for Home (%s shock)', shock_scenario);
...
h_foreign = figure('Name', 'IT_CPI: Foreign Welfare', 'Visible', 'off');
plot(phi_pi_H_range, results_welfare_F, 'b-', 'LineWidth', 2);
hold on;
plot(final_results.opt_phi_pi_H_F, final_results.max_welfare_F, 'r*', 'MarkerSize', 12, 'LineWidth', 1.5);
grid on;
xlabel('\phi_{\pi}^{H} (Normalized CPI)'); ylabel('Welfare');
line1_F = sprintf('IT_{CPI}: Optimal for Foreign (%s shock)', shock_scenario);
...
h_global = figure('Name', 'IT_CPI: Global Welfare', 'Visible', 'off');
plot(phi_pi_H_range, results_welfare_SUM, 'b-', 'LineWidth', 2);
hold on;
plot(final_results.opt_phi_pi_H_SUM, final_results.max_welfare_SUM, 'r*', 'MarkerSize', 12, 'LineWidth', 1.5);
grid on;
xlabel('\phi_{\pi}^{H} (Normalized CPI)'); ylabel('Welfare');
line1_G = sprintf('IT_{CPI}: Optimal for Global (Sum) (%s shock)', shock_scenario);
...
fprintf('\n====================================================\n');
fprintf('IT_CPI Policy optimization has finished for %s.\n', shock_scenario);
