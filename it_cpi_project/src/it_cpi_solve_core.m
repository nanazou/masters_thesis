function [welfares, irf_results, Mbase_, oobase_] = it_cpi_solve_core(phi_pi_H_value, settings)

% src/it_cpi_solve_core.m
% =========================================================================
% Solver実行、変数展開、Welfare計算
% 修正方針: Naming Convention (star at end of attribute, ss at very end)
% =========================================================================

global M_ oo_;

% --- [1] カレントディレクトリの保存と移動 ---
% .modファイルがある src フォルダへ一時的に移動する
root_dir = pwd;
...
% 定常状態パラメータの命名規則を修正 (pi_H_W_ss)
assignin('base', 'pi_H_W_ss', 1.0); 

% --- [3] シミュレーション設定の展開 ---
nperiods = settings.nperiods;
maxiter = settings.maxiter;
modnam = settings.modnam;
modnamstar = settings.modnamstar;
...
% --- [4] OccBinソルバーの実行 ---
fprintf('OccBinソルバー (IT-CPI) を実行中...\n');
...
% 2. 定常状態（Steady State）の取得 (命名規則修正版を参照)
...
% 政策評価用の正規化指数の定常状態
p_H_W_bar_ss_val      = oobase_.dr.ys(strcmp('p_H_W_bar', Mbase_.endo_names));
p_F_W_star_bar_ss_val = oobase_.dr.ys(strcmp('p_F_W_star_bar', Mbase_.endo_names)); % 【修正】命名規則を一致
