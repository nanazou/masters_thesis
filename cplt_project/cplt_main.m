% cplt_main.m
% このスクリプトは cplt_solve_core.m を呼び出し、特定のパラメータでの結果を詳細に出力します。
...
% ▼▼▼【計算に使用するパラメータ設定 (CPLT用)】▼▼▼
phi_gap_H_value = 51000;
phi_level_H_value = 100000;
% ▲▲▲【設定完了】▲▲▲
...
for s_idx = 1:length(scenarios)
    ...
    % --- 出力先フォルダの設定 (results > [shock] 直下) ---
    output_root_folder = fullfile(mpath, 'results', current_scen.name); 
    if ~exist(output_root_folder, 'dir'), mkdir(output_root_folder); end

    % --- 【核心】cplt_solve_core.m の呼び出し ---
    [welfares, irf_results, Mbase_, oobase_] = cplt_solve_core(phi_gap_H_value, phi_level_H_value, settings);
...
% 表示変数のリスト (chi_H および 統計ベース物価指数を補完)
    ...
    for i = 1:length(var_list)
        var_name = var_list{i};
        if ~strncmp(var_name, 'eps_', 4) 
            vars_to_save{end+1} = [var_name, '_pie']; 
            vars_to_save{end+1} = [var_name, '_lin']; 
        end
    end
    ...
    % 指定通り、results/[shock]/ 直下の data_for_plotting.mat に保存
