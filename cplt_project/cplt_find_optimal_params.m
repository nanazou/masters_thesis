% cplt_find_optimal_params.m
% このスクリプトは、CPLT政策の最適パラメータを探索し、
% 厚生の表面グラフを出力します。計算の核心部は src/cplt_solve_core.m を呼び出します。
...
fprintf('====================================================\n');
fprintf('Starting optimization for CPLT Policy...\n');
fprintf('====================================================\n\n');
...
phi_gap_H_range_coarse  = 0.00000000001:0.00000000001:0.00000000003;
phi_level_H_range_coarse = 0.000000000001:0.000000000001:0.000000000003;

% --- 4. 粗い探索 (coarse mode) の設定 --- beta final
%phi_gap_H_range_coarse  = 50000:1000:52000;
%phi_level_H_range_coarse = 90000:10000:150000;
...
center_phi_gap_H  = 199;
center_phi_level_H = 30.0;
fine_search_radius_gap_H   = 0.2;
fine_search_step_gap_H     = 0.05;
fine_search_radius_level_H = 0.05;
...
% 統一された CPLT モデルファイルを参照
settings.modnam = 'cplt_model';
settings.modnamstar = 'cplt_model_zlb';
...
for i = 1:length(phi_level_H_range_final)
    for j = 1:length(phi_gap_H_range_final)
        ...
        % --- [計算核心 cplt_solve_core を関数として呼び出す] ---
        try
            % 命名規則を統一したコアエンジンを呼び出し
            res = cplt_solve_core(phi_gap_H_value, phi_level_H_value, settings);
...
% =========================================================================
fprintf('\n--- CPLT Final Optimal Policy Results (Shock: %s) ---\n', shock_scenario);
...
h_home = figure('Name', 'CPLT: Home Welfare', 'Visible', 'off');
...
line2_H = sprintf('\\phi_{gap}^H=%.12f, \\phi_{level}^H=%.12f, Welfare=%.12f', ...
...
h_foreign = figure('Name', 'CPLT: Foreign Welfare', 'Visible', 'off');
...
line2_F = sprintf('\\phi_{gap}^H=%.12f, \\phi_{level}^H=%.12f, Welfare=%.12f', ...
...
h_global = figure('Name', 'CPLT: Global Welfare', 'Visible', 'off');
...
line2_G = sprintf('\\phi_{gap}^H=%.12f, \\phi_{level}^H=%.12f, Welfare=%.12f', ...
...
fprintf('\n====================================================\n');
fprintf('CPLT Policy optimization has finished for %s.\n', shock_scenario);
