% ngdplt_find_optimal_params.m
% このスクリプトは、NGDPLT政策の最適パラメータを探索し、
% 厚生の表面グラフを出力します。計算の核心部は src/ngdplt_solve_core.m を呼び出します。
...
fprintf('====================================================\n');
fprintf('Starting optimization for NGDPLT Policy...\n');
fprintf('====================================================\n\n');
...
%phi_gap_H_range_coarse   = 6.4:0.1:6.7;
%phi_level_H_range_coarse = 13.6:0.1:14.0;

% --- 4. 粗い探索 (coarse mode) の設定 --- a final
%phi_gap_H_range_coarse   = 6.4:0.1:6.6;
%phi_level_H_range_coarse = 13:0.5:15;

% --- 5. 詳細な探索 (fine mode) の設定 --- beta final
%center_phi_gap_H   = 6.50; 
%center_phi_level_H = 13.8; 
%fine_search_radius_gap_H   = 0.2; 
%fine_search_step_gap_H     = 0.1; 
%fine_search_radius_level_H = 0.1; 
%fine_search_step_level_H   = 0.05;

% --- 5. 詳細な探索 (fine mode) の設定 --- a final
%center_phi_gap_H   = 6.50; 
%center_phi_level_H = 13.9; 
%fine_search_radius_gap_H   = 0.2; 
%fine_search_step_gap_H     = 0.1; 
%fine_search_radius_level_H = 0.1; 
%fine_search_step_level_H   = 0.05;
...
for i = 1:length(phi_level_H_range_final)
    for j = 1:length(phi_gap_H_range_final)
        ...
        % --- [計算核心 ngdplt_solve_core を呼び出す] ---
        try
            % 接頭辞を統一したコアエンジンを呼び出し
            res = ngdplt_solve_core(phi_gap_H_value, phi_level_H_value, settings);
...
% =========================================================================
fprintf('\n--- NGDPLT Final Optimal Policy Results (Shock: %s) ---\n', shock_scenario);
...
h_home = figure('Name', 'NGDPLT: Home Welfare', 'Visible', 'off');
...
h_foreign = figure('Name', 'NGDPLT: Foreign Welfare', 'Visible', 'off');
...
h_global = figure('Name', 'NGDPLT: Global Welfare', 'Visible', 'off');
...
fprintf('\n====================================================\n');
fprintf('NGDPLT Policy optimization has finished for %s.\n', shock_scenario);
