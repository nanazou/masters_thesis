...
%   src フォルダ内の 'tr_cpi_stage1_main.m' と 'tr_cpi_stage2_main.m' を順番に呼び出します。
%   各ステージの結果は、results フォルダ内の各ショック専用フォルダに集約されます。
%   ※Stage 2 では Stage 1 で計算された「正規化CPIインフレ目標下の潜在産出量」が読み込まれます。
...
fprintf('========================================================================\n');
fprintf('         Taylor Rule (TR CPI) プロジェクト 全自動シミュレーション開始\n');
fprintf('========================================================================\n');

% シナリオ（Beta と A）のループを開始
for s = 1:length(scenarios)
    
    % 子スクリプト（tr_cpi_stage1_main, tr_cpi_stage2_main）に渡す共通変数をセット
    ...
    % このステップで、中央銀行が正規化CPI（値札ベース）に反応する環境下での「自然水準」を算出します。
    fprintf('\n>> Step 1: src/tr_cpi_stage1_main.m を実行しています (潜在産出量パスの計算)...\n');
    try
        % 'run' コマンドにより、現在のワークスペースの変数(shock_scenario)を維持したまま実行
        run(fullfile(root_mpath, 'src', 'tr_cpi_stage1_main.m'));
    catch ME
    ...
    % Stage 1 で保存された 'potential_output.mat' を基に、出力ギャップを考慮した政策シミュレーションを行います。
    fprintf('\n>> Step 2: src/tr_cpi_stage2_main.m を実行しています (政策シミュレーション)...\n');
    try
        % Stage 1 で保存された 'potential_output.mat' が自動的にロードされます
        run(fullfile(root_mpath, 'src', 'tr_cpi_stage2_main.m'));
    catch ME
