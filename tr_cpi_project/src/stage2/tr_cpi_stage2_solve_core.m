function [welfares, irf_results, Mbase_, oobase_] = tr_cpi_stage2_solve_core(phi_pi_H_value, phi_y_H_value, settings)

% src/stage2/tr_cpi_stage2_solve_core.m
% =========================================================================
% Solver実行、変数展開、Welfare計算 (TR CPI用 - 潜在産出量対応版)
% 
% 目的:
%   1. OccBinを使用して、ZLBを考慮した消費者物価目標政策をシミュレートする。
...
modnam = settings.modnam;         % 'tr_cpi_stage2_model'
modnamstar = settings.modnamstar; % 'tr_cpi_stage2_model_zlb'
...
% --- [4] OccBinソルバーの実行 ---
fprintf('OccBinソルバー (TR CPI Stage 2) を実行中...\n');
