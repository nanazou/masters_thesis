% it_ppi_main.m
% このスクリプトは it_ppi_solve_core.m を呼び出し、特定のパラメータでの結果を詳細に出力します。
...
%% --- 共通シミュレーション設定 ---
% 独自名に変更した統一ファイル名 ('it_ppi_model', 'it_ppi_model_zlb') を参照
...
for s_idx = 1:length(scenarios)
    ...
    % --- 【核心】it_ppi_solve_core.m の呼び出し ---
    [welfares, irf_results, Mbase_, oobase_] = it_ppi_solve_core(phi_pi_H_value, settings);
